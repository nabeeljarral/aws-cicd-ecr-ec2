...
import {checkReportCompleted, downloadReport, reconReport} from '@/utils/services/reports';
...
const ReconReport = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await reconReport(filter);
                ...
                const id = setInterval(() => {
                    checkReportCompleted(res._id).then((res) => {
                        if (res && res.fileAttached) {
                            ...
                            downloadReport(res._id, 'recon-report').then((res) => {
                                setLoading(false);
                            });                           }
                    });
                }, 10000);
    ...
    useEffect(() => {
        if (!roles?.includes(RoleEnum.ReconReport)) router.push(LOGIN_ROUTE);
    }, [roles]);
    useEffect(() => {
        return () => {
