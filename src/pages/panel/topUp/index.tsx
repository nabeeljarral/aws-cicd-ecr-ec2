...
const TopUpPage = (props: any) => {
    ...
    const fetchTransferHistory = async (f?: Partial<IBalanceHistory>) => {
        ...
        // @ts-ignore
        filter = {...filter, type: {$in: [BalanceTypeEnum.TopUp]}};
    ...
    const exportReport = async () => {
        setLoading(true)
        const payload = {...formData,type:BalanceTypeEnum.TopUp,relatedTo:userId};
        try {
                    ...
                    checkReportCompleted(res._id).then(async (res) => {
                        if (res && res.fileAttached) {
                            if (id !== null) clearInterval(id);
                                downloadReport(res._id, `${BalanceTypeEnum.TopUp}`, 'batch').then(
                                    (res) => {
                                        setLoading(false);
                                    },
                                );
    ...
    useEffect(() => {
        if (!roles?.includes(RoleEnum.ClientTopUp)) router.push(LOGIN_ROUTE);
    }, [roles]);
    return (
        <DashboardLayout>
            <Box sx={{flexGrow: 1}}>
