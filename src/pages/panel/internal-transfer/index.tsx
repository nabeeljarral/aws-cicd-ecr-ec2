...
const InternalTransferPage = (props: any) => {
    ...
    const fetchTransferHistory = async (f?: Partial<IBalanceHistory>) => {
        ...
        // @ts-ignore
        filter = {...filter, type: {$in: [BalanceTypeEnum.InternalTransfer]}};
    ...
    const exportReport = async () => {
        setLoading(true);
        const payload = {...formData, type: BalanceTypeEnum.InternalTransfer, relatedTo: userId};
        try {
                ...
                const id = setInterval(() => {
                    checkReportCompleted(res._id).then(async (res) => {
                        if (res && res.fileAttached) {
                            ...
                            downloadReport(
                                res._id,
                                `${BalanceTypeEnum.InternalTransfer}`,
                                'batch',
                            ).then((res) => {
                                setLoading(false);
                            });
    ...
    useEffect(() => {
        if (!roles?.includes(RoleEnum.ClientInternalTransfer)) router.push(LOGIN_ROUTE);
    }, [roles]);
    return (
        <DashboardLayout>
            <Box sx={{flexGrow: 1}}>
