...
import {updateBulkUtr} from '@/utils/services/bulk-utr';
...
const BulkUpdateUTR = (props: any) => {
    ...
    const submitUpdateCreateGatewayAccount = async (payload: ITransformedBulkUpdate) => {
        ...
        try {
            ...
            const response = await updateBulkUtr('', payload);
    ...
    const handleResponse = (response: any) => {
        ...
        if (response.ids) {
            ...
            awesomeAlert({msg:`All done | Total : ${responseData?.total} | Modified : ${response?.modifiedCount}`});
                                ...
                                <Grid container spacing={3} direction="row" wrap="wrap">
                                    ...
                                    <Grid item xs={12} sm={6} md={4}>
                                        <TextField
                                            ...
                                            multiline
                                            rows={5}
                                            placeholder='Enter the transaction UTRs as comma separated e.g.: UTR,UTR,UTR,...'
                                        />
                                    </Grid>
                                    <Grid item xs={12} sm={6} md={4}>
                                        <TextField
                                            ...
                                            multiline
                                            rows={5}
                                            placeholder='Enter the transaction Amounts as comma separated e.g.: Amount,Amount,Amount,...'
                                        />
                                    </Grid>
                                </Grid>
                                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                                    ...
                                    {responseData.ids && !responseData?.success && (
                                        ...
                                        >
                                            <Alert
                                                severity="warning"
                                                sx={{background:"#fff8db"}}
                                            >
                                                Some of the UTRs and Amounts are not Updated, Please
                                                verify the UTRs and Amounts, Modified :<span style={{fontWeight:"600",borderRight:"1px solid black",paddingRight:"3px"}}>{responseData?.modifiedCount}</span>  Not Modified : <span style={{fontWeight:"600"}}>{responseData?.notModifiedCount}</span>
                                            </Alert>
                                            <Alert
                                                severity="info"
                                            >
                                                Unupdated UTRs and Amounts are populated at their fields,
                                                please review and proceed & Add again
                                            </Alert>
                                        </Box>
                                    )}
                                </Box>
